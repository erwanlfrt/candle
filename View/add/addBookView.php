...
<html>
  ...
  <body>
      ...
      ?>
    <div id="main">
      <form action="?action=add&table=book&control" method="post">
        <div>
            <input class="textField" type="text" id="title" name="title" placeholder="title">
            <input class="textField" type="text" id="author" name="author" placeholder="author">
        </div>
        <div class="button">
        ...
        <?php
            if(isset($_GET['erreur'])){
                ...
                if($err==1 || $err==2)
                {
                  echo "<p style='color:red'>Book already exist</p>";
                }
                elseif($err==3){
                    echo "<p style='color:red'>Invalid form</p>";
                }
                elseif($err==4){
                    echo "<p style='color:red'>Author unknown</p>";
                }
            }
        ?>
      </div>
